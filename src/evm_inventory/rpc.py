...
class RpcReader:
    ...
    def block(self, url, chain_id, number=None):
        ...
        result = self.call(
            url, "eth_getBlockByNumber", [hex(number) if number is not None else "latest", False]
        )
    ...
    def token(self, url, chain_id, wallet, contract, block, expected_decimals=None):
        ...
        if key not in self.metadata:
            ...
            try:
                decimals = uint256(
                    self.call(url, "eth_call", [{"to": contract, "data": "0x313ce567"}, hex(block)])
                )
                if decimals > 255:
        ...
        raw = uint256(
            self.call(
                url, "eth_call", [{"to": contract, "data": balance_of_data(wallet)}, hex(block)]
            )
        )
        return raw, decimals
