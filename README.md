...
Проверьте JSON плана, затем запустите выполнение с обязательным `--execute`:
...
После отправки исполнитель ждёт, пока Bitget увидит депозит. Если запись появилась со статусом `pending`, ожидание окончательного `success` продолжается в фоне, а исполнитель переходит к следующей транзакции. Если Bitget не увидел депозит за 30 минут, сообщил об ошибке либо не подтвердил зачисление за 6 часов, дальнейшая отправка останавливается и причина сохраняется в журнале. Повторный запуск того же плана с тем же `--db` сверяет сохранённый хеш без новой подписи. Новый план с тем же кошельком, сетью и активом не отправляется, пока прежний депозит не закрыт. Это правило также позволяет сверить ошибочно помеченные ранее `manual_review` прямые депозиты, у которых в журнале сохранён хеш.

Для Ethereum Mainnet верхняя цена газа EIP-1559 рассчитывается как `ceil(baseFee × 1.3)` и по-прежнему проверяется против настроенного лимита.
...
При ответе Rabby HTTP 429 каждый GET повторяется ещё три раза с экспоненциальными паузами 5, 10 и 20 секунд. Если сервер прислал `Retry-After`, он задаёт нижнюю границу первой паузы; следующие паузы удваиваются. После исчерпания попыток действие передаётся на `manual_review`, а не в `skipped`; ошибка 429 при построении плана переносит весь кошелёк на повторную попытку. Ответы 429 и сроки повторов записываются в лог.
